...
import { Box, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, Button, TextField, MenuItem, Stack, Dialog, DialogTitle, DialogContent, DialogActions } from '@mui/material';
...
export function ExpensesReportView() {
  ...
  const pieDataByUser = useMemo(()=>
    USERS.map(user=>{
      const userExpenses = expenses.filter(e=>e.paid_by===user);
      ...
      return {user,data:pieData,total};
    }), [expenses]
  );
  const totalGeral = pieDataByUser.reduce((acc,user)=>acc+user.total,0);
  const formatEuro = (value:number)=>value.toLocaleString('pt-PT',{style:'currency',currency:'EUR'});

  return (
    <LocalizationProvider dateAdapter={AdapterDateFns}>
      <Box sx={{p:3}}>
        <Typography variant="h4" sx={{mb:3}}>Relatório de Gastos</Typography>
        ...
        <Box sx={{display:'flex',gap:2,flexWrap:'wrap',mb:2}}>
          ...
          <Button variant="contained" onClick={fetchExpenses}>Atualizar</Button>
        </Box>
                ...
                <TableRow key={exp.id}>
                  ...
                  <TableCell>
                    <Stack direction="row" spacing={1}>
                      <Button variant="outlined" size="small" onClick={()=>handleOpenEdit(exp)}>Editar</Button>
                      <Button variant="outlined" color="error" size="small" onClick={()=>handleDelete(exp.id)}>Deletar</Button>
                    </Stack>
                  </TableCell>
                </TableRow>
              ))}
        ...
        <Typography variant="h6" sx={{mb:2}}>Total Geral: {formatEuro(totalGeral)}</Typography>

        <Box sx={{display:'flex',gap:4,flexWrap:'wrap'}}>
          {pieDataByUser.map(p=>(
            <Box key={p.user} sx={{width:300,height:350}}>
              <Typography variant="h6" sx={{textAlign:'center',mb:1}}>{p.user} - Total: {formatEuro(p.total)}</Typography>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie data={p.data} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label>
                    {p.data.map((entry,index)=><Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>)}
                  </Pie>
        ...
        <Dialog open={openEdit} onClose={handleCloseEdit}>
          ...
          <DialogContent sx={{display:'flex',flexDirection:'column',gap:2,mt:1}}>
            <TextField
              ...
              onChange={e=>setEditExpense(prev=>prev?{...prev,title:e.target.value}:null)}
            />
            <TextField
              select
              label="Categoria"
            ...
            <DatePicker
              ...
              onChange={d=>setEditExpense(prev=>prev&&d?{...prev,date:d.toISOString()}:prev)}
            ...
            <TextField
              ...
              onChange={e=>setEditExpense(prev=>prev?{...prev,paid_by:e.target.value}:null)}
            >
              {USERS.map(u=><MenuItem key={u} value={u}>{u}</MenuItem>)}
            </TextField>
          </DialogContent>
